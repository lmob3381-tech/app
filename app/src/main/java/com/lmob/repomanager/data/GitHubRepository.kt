...
class GitHubRepository(private val api: GitHubApi) {
    ...
    /**
     * Setara dengan bersihkan-repo.sh: hapus semua file di repo lewat Git Data API
     * (tree + commit + update ref) dalam SATU commit atomik, supaya cepat dan tidak
     * bisa "gagal separuh jalan". File/folder penting seperti .git, .github, .gitignore,
     * dan .gitattributes TIDAK ikut dihapus.
    ...
    suspend fun cleanRepo(
        ...
        try {
            onProgress("Mengambil referensi branch $branch...")
            ...
            onProgress("Membaca daftar file...")
            ...
            val protectedPrefixes = listOf(".git/", ".github/")
            val protectedNames = setOf(".gitignore", ".gitattributes", ".gitmodules")

            val files = treeResp.body()!!.tree.filter { entry ->
                entry.type == "blob" &&
                    protectedNames.none { entry.path == it } &&
                    protectedPrefixes.none { entry.path.startsWith(it) }
            }

            if (files.isEmpty()) {
                onProgress("Tidak ada file yang perlu dihapus (selain .git/.github).")
                return@withContext Result.Success(0)
            }

            for (item in files) {
                onProgress("Menandai hapus: ${item.path}")
            }

            // Tandai setiap file untuk dihapus dengan sha = null pada tree baru,
            // sisanya (mis. .github/**) otomatis dipertahankan karena tidak disebut ulang.
            // Pakai GhDeleteTreeItem (tanpa field `content` sama sekali) supaya tidak pernah
            // bentrok dengan validasi GitHub "Must supply either tree.sha or tree.content".
            val newTreeItems = files.map { entry ->
                GhDeleteTreeItem(path = entry.path, mode = entry.mode, type = "blob", sha = null)
            }

            onProgress("Membuat commit penghapusan...")
            val treeCreateResp = api.createDeleteTree(owner, repo, GhDeleteTreeRequest(baseTree = headSha, tree = newTreeItems))
            ...
            val commitResp = api.createCommit(
                owner, repo,
                GhNewCommitRequest(message = "chore: kosongkan isi repo (kecuali .git/.github)", tree = newTreeSha, parents = listOf(headSha))
            )
            ...
            if (!updateResp.isSuccessful) {
                return@withContext Result.Error(
                    "Commit dibuat (${newCommitSha.take(7)}) tapi gagal update branch $branch: " +
                        errorMessage(updateResp.code(), updateResp.errorBody()?.string())
                )
            }

            // Verifikasi: GitHub Contents API kadang butuh sesaat untuk konsisten setelah
            // commit baru. Kita cek ulang HEAD ref supaya log menunjukkan status sebenarnya.
            onProgress("Memverifikasi HEAD branch $branch...")
            val verifyRef = api.getBranchRef(owner, repo, branch)
            val verifiedSha = verifyRef.body()?.`object`?.sha
            if (verifyRef.isSuccessful && verifiedSha != newCommitSha) {
                onProgress(
                    "⚠ HEAD $branch masih menunjuk ke ${verifiedSha?.take(7) ?: "?"}, " +
                        "bukan commit baru ${newCommitSha.take(7)}. Kemungkinan ada commit lain " +
                        "(mis. dari GitHub Actions) yang menimpa. Coba jalankan Bersihkan lagi."
                )
            } else {
                onProgress("HEAD $branch terkonfirmasi di commit ${newCommitSha.take(7)}.")
            }

            onProgress("Selesai. ${files.size} file dihapus.")
